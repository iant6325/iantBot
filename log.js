...
module.exports = function (Client) {
    ...
    Client.on('guildMemberAdd', member => {
        serversettingsModel.findOne({ serverId: member.guild.id }).then(server => {
            ...
            if (server.logchannelId != "0") Client.channels.get(server.logchannelId).send(
    ...
    Client.on('messageDelete', message => {
        serversettingsModel.findOne({ serverId: message.channel.guild.id }).then(server => {
            ...
            if (server.logchannelId != "0" && !message.author.bot) Client.channels.get(server.logchannelId).send(
                new Discord.RichEmbed()
